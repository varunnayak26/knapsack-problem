/*
 * Java Genetic Algorithm Library (jenetics-5.0.1).
 ...
 * @version 4.0
 */
public abstract class CustomRecombinator<
		G extends Gene<?, G>,
		...
		extends AbstractAlterer<G, C>
{
	 ...
	 *         smaller than two.
	 */
	protected CustomRecombinator(final double probability, final int order) {
		super(probability);
		if (order < 2) {
	...
	public final AltererResult<G, C> alter(
		...
		if (population.size() >= 2) {
			...
			final int count = indexes(random, population.size(), _probability)
					...
					.peek(i -> System.out.println( Arrays.toString(i)))
